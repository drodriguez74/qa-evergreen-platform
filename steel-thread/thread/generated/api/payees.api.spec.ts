...
const PayeesListSchema = z.array(PayeeSchema);
...
test.describe('payees', () => {
  ...
  test('GET /api/payees → 200 returns array of payees', async () => {
    const res = await apiContext.get('/api/payees', {
      headers: { Authorization: `Bearer ${token}` },
    });
    expect(res.status()).toBe(200);
    const body = await res.json();
    PayeesListSchema.parse(body);
  });
});
